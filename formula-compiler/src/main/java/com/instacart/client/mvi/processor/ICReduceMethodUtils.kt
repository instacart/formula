...
object ICReduceMethodUtils {
    fun findReduceMethods(reducerClass: TypeElement): Either<KnownError, ReduceMethods> {
        ...
        if (validMethods.isEmpty()) {
            return KnownError("${reducerClass.simpleName} needs to have at least 1 reduce method").left()
        }
        ...
        if (!allMethodsReturnSameType) {
            ...
            return KnownError("${reducerClass.simpleName} has mismatching type in the reduce function, types: $types").left()
        }

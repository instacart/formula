...
import com.instacart.client.mvi.state.ICNext
...
object ICProcessorUtils {
    ...
    fun asReducerClass(reducerClassElement: TypeElement): Either<KnownError, Option<NextReducerClass>> {
        return if (isNotEmptyReducerClass(reducerClassElement)) {
            ICReduceMethodUtils.findReduceMethods(reducerClassElement).map {
                val effectType = ICProcessorUtils.parseEffectType(it)
    ...
    fun nextReducerType(stateClassType: TypeName, effectType: TypeName): TypeName {
        return Function1::class.asTypeName().parameterizedBy(
            stateClassType,
            ICNext::class.asTypeName().parameterizedBy(stateClassType, effectType)
        )
    }

    private fun isNotEmptyReducerClass(element: TypeElement): Boolean {
        return !element.asType().asTypeName().toString().contains(ICNextReducers::class.asTypeName().toString())
    }

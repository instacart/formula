...
object ICDirectInputCodeGenerators {
    class Code(
        val relayProperties: List<PropertySpec>,
        val relayAccessorFunctions: List<FunSpec>,
        val bindStatements: List<ICReducerBindStatement>
    )
    ...
    fun generateCode(
        ...
        return Code(
            ...
            bindStatements = directInputMethods.map {
                ICReducerBindStatement.localRelay(name = it.name, reducerVariableName = reducerVariableName)
            }
        )
    }
}
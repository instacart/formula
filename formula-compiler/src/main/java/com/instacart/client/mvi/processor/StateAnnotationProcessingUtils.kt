...
import com.instacart.client.mvi.DirectInput
...
object StateAnnotationProcessingUtils {
    ...
    fun handleStateAnnotatedType(
        ...
        return ICProcessorUtils.asReducerClass(reducerFactory).flatMap {
            val exportedStateProperties = findExportedProperties(stateElement, stateConstructorParamNames)
            val parsedReducerClass = it.orNull()
            if (parsedReducerClass == null && exportedStateProperties.isEmpty()) {
                return KnownError("need to provide @State(reducers = ReducerClass) or mark some properties with @ExportedProperty").left()
            }
            ...
            val generated = exportedStateProperties.ifNotEmpty {
                generateReducerClass(prefix, stateElement, exportedStateProperties, packageName)
            }
            val generatedNextReducer = generated?.let { asNextReducerClass(it, effectType) }

            val eventsClassFile = ICMviEventClassGenerator.createFile(
    ...
    private fun asNextReducerClass(
        generatedBaseClass: StateAnnotationProcessingUtils.GeneratedReducerClass,
        effectType: TypeName
    ): NextReducerClass {
        val reducerType = ICProcessorUtils.nextReducerType(generatedBaseClass.stateType, effectType)
    ...
    private fun generateReducerClass(
        ...
        val reduceMethods = exportedStateProperties.map {
            Pair(it, generateReducerFunction(it, stateClassType, effectTypeVariable))
        }
        val generatedReducerClassType = TypeSpec.classBuilder(fileName)
            .addTypeVariable(effectTypeVariable)
            .superclass(
                ICNextReducers::class.asTypeName().parameterizedBy(
                    stateClassType,
                    effectTypeVariable
                )
            )
    ...
    private fun generateReducerFunction(
        ...
        return FunSpec.builder(functionName)
            ...
            .returns(ICProcessorUtils.nextReducerType(stateClassType, effectType))
            .build()
    }
}
